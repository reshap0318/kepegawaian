<div class="mt-4">
    ...
    <div class="grid grid-cols-3 pb-3">
        ...
        @can('apiAksess_manage')
            <div class="text-right col-span-2">
                <x-table.button class="inline-flex items-center text-sm font-medium rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 px-1.5" color="indigo" href="{{ route('apiAksess.create') }}">
    ...
    <div x-data="{show:false}">
        <x-table.init>
            ...
            <x-table.tbody>
                @foreach ($apiAksess as $apiAkses)
                    <x-table.tr>
                        <x-table.td>{{ $apiAkses->nama }}</x-table.td>
                        <x-table.td>{{ $apiAkses->app_key }}</x-table.td>
                        <x-table.td>{{ $apiAkses->created_at }}</x-table.td>
                        <x-table.td class="text-right">
                            @can('apiAksess_manage')
                            <div class="flex space-x-1 justify-end">
                                <x-table.button class="normal-case" color="indigo" href="{{ route('apiAksess.edit',$apiAkses) }}">
                                    <svg class="h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </x-table.button>
                                <x-table.button class="normal-case" color="red" @click="show = true" wire:click="deleteModel({{ $apiAkses }})">
                                    <svg class="h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
