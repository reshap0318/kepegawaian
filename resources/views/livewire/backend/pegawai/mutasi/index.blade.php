<div class="mt-4">
    ...
    <div class="grid grid-cols-3 pb-3">
        <h2 id="informasi Fungsional" class="text-lg leading-6 font-medium text-gray-900">Riwayat Mutasi</h2>
        <div class="text-right col-span-2">
            <x-form.button px="2" py="1" color="indigo" href="{{ route('pegawaiMutasis.create', $user) }}">
                    ...
                    @foreach ($pegawaiMutasis as $pegawaiMutasi)
                        <x-table.tr>
                            <td class="pl-6 py-4" style="width: 3px">
                                @if ($pegawaiMutasi->status)
                                    <span class="h-5 w-5 rounded-full bg-green-500 flex items-center justify-center ring-5 ring-white" wire:click="changeStatus({{ $pegawaiMutasi }})">
                                ...
                                @else
                                    <span class="h-5 w-5 rounded-full bg-gray-500 flex items-center justify-center ring-5 ring-white" wire:click="changeStatus({{ $pegawaiMutasi }})">
                                        <!-- Heroicon name: solid/check -->
                            ...
                            <x-table.td>{{ $pegawaiMutasi->tgl_mutasi }}</x-table.td>
                            <x-table.td>{{ $pegawaiMutasi->unit->nama }}</x-table.td>
                            <x-table.td>
                                <x-form.button class="normal-case" px="3" py="1" color="indigo" href="{{ $pegawaiMutasi->file_sk_url }}" target="blank">
                                ...
                                <x-form.button class="normal-case" px="3" py="1" color="red" @click="show = true" wire:click="deleteModel({{ $pegawaiMutasi }})">
                                    <svg class="h-3 w-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
