<div class="mt-4">
    ...
    <div class="grid grid-cols-3 pb-3">
        ...
        <div class="text-right col-span-2 flex space-x-1 justify-end">
            @can('pegawai-fungsional_manage')
                <x-table.button color="indigo" href="{{ route('pegawaiFungsionals.create', $user) }}" class="px-3">
                ...
                @foreach ($pegawaiFungsionals as $pegawaiFungsional)
                    <x-table.tr>
                        ...
                        <div class="flex space-x-1 justify-end">
                            <x-table.button color="indigo" href="{{ $pegawaiFungsional->file_sk_url }}" target="blank">
                                <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                                </svg>
                            </x-table.button>
                            @can('pegawai-fungsional_manage')
                            <x-table.button color="indigo" @click="showConfirm = true" wire:click="changeStatusModel({{ $pegawaiFungsional }})">
                                <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                            </x-table.button>
                            <x-table.button href="{{ route('pegawaiFungsionals.edit',['user' => $user, 'pegawaiFungsional' => $pegawaiFungsional]) }}">
                                <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                </svg>
                            </x-table.button>
                            <x-table.button color="red" @click="show = true" wire:click="deleteModel({{ $pegawaiFungsional }})">
